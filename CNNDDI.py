...
from sklearn.linear_model import LogisticRegression # LJ
...
def prepare(df_drug, feature_list, vector_size, mechanism, action, drugA, drugB):
    ...
    # Count the number of each interaction event (mechanism+action)
    d_event=[]
    ...
    # Sort the events in reverse order of their counts
    list1 = sorted(count.items(), key=lambda x: x[1],reverse=True)
    
    # key = event, value = index of the event
    for i in range(len(list1)):
        d_label[list1[i][0]] = i
    
    # Build feature vector by horizontally stacking the feature vector of each feature
...
def feature_vector(feature_name, df):
    ...
    # Feature values for each drug, for example, when feature_name is target, drug_list=["P30556|P05412","P28223|P46098|……"]
    for i in drug_list:
    ...
    df_feature = DataFrame(feature_matrix, columns=all_feature)  # Consrtuct binary feature matrices with key of dataframe
